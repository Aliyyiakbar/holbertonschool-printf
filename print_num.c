...
/**
 * pf_num_out - output number with padding/precision/prefix/sign
 * @b: buffer
 * @f: format
 ...
 */
static int pf_num_out(pf_buffer_t *b, pf_format_t *f, char *digits, int dlen,
		      char sign, const char *pref, int plen)
{
	int zeros, total, pad;
	char padc;
...
/**
 * pf_print_signed - prints %d/%i with flags/width/precision and l/h
 * @b: buffer
 * @f: format
 * @ap: args
 *
...
int pf_print_signed(pf_buffer_t *b, pf_format_t *f, va_list *ap)
{
	...
	if (num == 0 && f->precision == 0)
		return (pf_num_out(b, f, digits, 0, sign, "", 0));

	dlen = pf_build_digits(num, 10, 0, digits);
	return (pf_num_out(b, f, digits, dlen, sign, "", 0));
}

/**
 * pf_print_unsigned_base - prints %u/%o/%x/%X with flags/width/precision and l/h
 * @b: buffer
 * @f: format
 * @ap: args
 * @base: base
			   ...
			   int base, int upper)
{
	...
	if (num == 0 && f->precision == 0)
	{
		...
		{
			digits[0] = '0';
			return (pf_num_out(b, f, digits, 1, 0, "", 0));
		}
		return (pf_num_out(b, f, digits, 0, 0, "", 0));
	}

	dlen = pf_build_digits(num, base, upper, digits);
	return (pf_num_out(b, f, digits, dlen, 0, pref, plen));
}
...
int pf_print_binary(pf_buffer_t *b, pf_format_t *f, va_list *ap)
{
	...
	return (pf_num_out(b, f, digits, dlen, 0, "", 0));
}

/**
 * pf_print_pointer - prints %p (0x... or (nil))
 * @b: buffer
 * @f: format
 * @ap: args
 *
...
int pf_print_pointer(pf_buffer_t *b, pf_format_t *f, va_list *ap)
{
	...
	return (pf_num_out(b, f, digits, dlen, 0, "0x", 2));
}
